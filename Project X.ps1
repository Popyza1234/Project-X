...
function Start-BoosterBackground {
    ...
    # streams are sent to files. Redirecting them matters - a child that inherits
    ...
    try {
        ...
        $null = Start-Process -FilePath 'powershell.exe' -ArgumentList $bArgs -WindowStyle Hidden `
            -RedirectStandardOutput (Join-Path $BackupDir 'booster-stdout.log') `
            -RedirectStandardError (Join-Path $BackupDir 'booster-stderr.log')
    } catch { return $false }
    Start-Sleep -Seconds 2
    return (Test-BoosterRunning)
}
...
$script:Menu = @(
    ...
    @{ Key = '5'; Id = 'traces';  Title = 'TRACES';  Desc = 'erase the PROJECT X run history Windows keeps' }
)
...
#
#  Windows keeps several "what has been run" records. Only the records that name
#  THIS tool are removed; every other program keeps its history untouched.
...
function Remove-RunTraces {
    ...
    Write-Host '   . temp copy     %TEMP%\ProjectX\Project X.ps1 - that is what the exe runs' -ForegroundColor DarkGray
    Write-Host ''

    $left = Get-BamTraceCount
    ...
    Write-Host ('   . processed     ' + $total + ' record(s) in ' + [Math]::Round($sw.Elapsed.TotalSeconds, 1) + ' s') -ForegroundColor DarkGray
}
